...
public class JFXRepresentation extends ToolkitRepresentation<Parent, Node>
{
    ...
    /** Debounce timer for auto-zoom: fires one frame after the last resize event */
    private PauseTransition auto_zoom_timer;
    ...
    final public ScrollPane createModelRoot()
    {
        ...
        // One-shot timer used to defer auto-zoom until after the resize settles.
        // Duration.ONE = 1 ms — just enough to let the current pulse finish
        // before we read the stable viewport size.
        auto_zoom_timer = new PauseTransition(javafx.util.Duration.millis(50));
        auto_zoom_timer.setOnFinished(e -> applyAutoZoom());
    ...
    {
        // In auto-zoom mode widget_pane has no transform; the scale is on widget_parent.
        // Check widget_pane first (normal zoom), then fall back to widget_parent (auto-zoom).
        for (final Node node : new Node[]{ widget_pane, widget_parent })
        {
            final List<Transform> transforms = node.getTransforms();
    ...
    private void handleViewportChanges()
    {
        ...
        if (copy != null  &&  copy.propAutoZoomToFitAll().getValue())
        {
            // Auto-zoom mode: debounce — fire applyAutoZoom 50 ms after the
            // last resize event so the ScrollPane width/height has settled.
            auto_zoom_timer.playFromStart();
        }
        else
        {
            // Normal mode: release any auto-zoom constraints and restore
            // standard scrollbar / minSize behaviour.
            widget_pane.setMaxSize(Double.MAX_VALUE, Double.MAX_VALUE);
            widget_pane.getTransforms().clear();
            ...
            final int model_width  = (copy == null) ? 0 : copy.propWidth().getValue();
            final int model_height = (copy == null) ? 0 : copy.propHeight().getValue();
    ...
    /** Apply auto-zoom after viewport has settled (called by auto_zoom_timer). */
    private void applyAutoZoom()
    {
        ...
        final int mw = copy.propWidth().getValue();
        final int mh = copy.propHeight().getValue();
        if (mw <= 0  ||  mh <= 0)
            return;

        // Read the ScrollPane outer size — stable, unaffected by content transforms.
        final double vw = model_root.getWidth();
        final double vh = model_root.getHeight();
        if (vw <= 0  ||  vh <= 0)
            return;

        // Zoom factor: 100% == native model dimensions fill the window.
        final double zoom = Math.min(vw / mw, vh / mh);

        // Scale ONLY widget_pane (not widget_parent) so the effective render
        // scale is zoom¹, not zoom².  widget_parent is a plain child of
        // widget_pane and inherits the pane's transform — no extra scale needed.
        widget_parent.getTransforms().clear();
        widget_pane.getTransforms().setAll(new Scale(zoom, zoom));

        // Size widget_pane to the scaled dimensions so the ScrollPane content
        // area matches the visible content exactly (no excess blank space when
        // zooming out, no spurious scrollbars when zooming in).
        widget_pane.setMaxSize(Double.MAX_VALUE, Double.MAX_VALUE);
        widget_pane.setMinSize(mw * zoom, mh * zoom);

        // Update the zoom percentage box (100% == native model size).
        if (zoom_listener != null)

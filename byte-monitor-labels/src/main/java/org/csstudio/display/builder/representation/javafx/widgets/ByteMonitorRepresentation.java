...
public class ByteMonitorRepresentation extends RegionBaseRepresentation<Pane, ByteMonitorWidget>
{
    ...
    // zyuan: store bit indices (0=off, 1=on) per LED for per-LED label text
    private volatile int[] value_indices = null;
    ...
    */
    ...
    private void addLEDs(final Pane pane, final double w, final double h, final boolean horizontal)
    {
        ...
        {
            // zyuan: Create label for dynamic on/off text or static label text
            ...
            // zyuan: Set initial label text (will be updated when value changes)
            if (use_dynamic_labels)
            {
                // Use off_label as default (no value yet)
                label.setText(off_label);
            }
            else
            {
                // Use static labels from the labels array
                final int lbl_index = bitReverse ? i : save_bits - i - 1;
                if (lbl_index < model_widget.propLabels().size())
                    label.setText(model_widget.propLabels().getElement(lbl_index).getValue());
            }
    ...
    {
        final int[] indices = computeColorIndices(new_value);
        final Color[] new_colorVals = new Color[indices.length];
        final Color[] save_colors = colors;
        for (int i = 0; i < indices.length; i++)
            new_colorVals[i] = save_colors[indices[i]];
        value_colors = new_colorVals;
        // zyuan: store per-bit indices so updateChanges can show per-LED labels
    ...
    public void updateChanges()
    {
        ...
        if (dirty_content.checkAndClear())
        {
            ...
            final int[] save_indices = value_indices;
            ...
            else
            {
                ...
                for (int i = 0; i < N; i++)
                {
                    ...
                    if (save_labels[i] != null)      // zyuan, changed here to avoid NPE when labels are added/removed while content changes
                    {
                        // zyuan: Set label text - dynamic on/off labels or static labels
                        if (use_dynamic_labels)
                        {
                            // zyuan: use index directly - 1=on, 0=off - per LED bit state
                            final boolean bit_on = (save_indices != null && i < save_indices.length) ? save_indices[i] != 0 : false;
                            save_labels[i].setText(bit_on ? on_label : off_label);
                        }
                        else
                        {
                            // Use static labels from the labels array
                            final int lbl_index = bitReverse ? i : numBits - i - 1;
                            if (lbl_index < model_widget.propLabels().size())
                                save_labels[i].setText(model_widget.propLabels().getElement(lbl_index).getValue());
                            else
                                save_labels[i].setText("");
                        }

 ...
 *
 *  <p>FNAL: Integrates with the global write-enable gate via
 *  {@link WritablePVWidget#addGateListener(Consumer)}.
 *
 *  @author Kay Kasemir
 *  @author FNAL (global write-enable gate)
 */
...
public class TableWidget extends VisibleWidget
{
    ...
    // -----------------------------------------------------------------------
    // FNAL global write-enable gate integration
    // -----------------------------------------------------------------------
    /** The user-configured (design-time) editable intent. */
    private volatile boolean userEditable = true;
    /** Guard against recursive listener calls when gate forces editable=false. */
    private volatile boolean applyingGate = false;
    /**
     * Strong reference to the gate callback — must be a field so the weak
     * reference held by WritablePVWidget is not prematurely collected.
     */
    private final Consumer<Boolean> gateListener = this::applyGlobalGate;

    /** Called by {@link WritablePVWidget#setGlobalWriteEnabled(boolean)}. */
    private void applyGlobalGate(final boolean enabled)
    {
        applyingGate = true;
        try { editable.setValue(enabled && userEditable); }
    ...
    protected void defineProperties(final List<WidgetProperty<?>> properties)
    {
        ...
        // FNAL: intercept every change to 'editable' so we can apply the global gate.
        // This also fires when the .bob file loads the design-time value.
        editable.addPropertyListener((prop, old, val) -> {
            if (applyingGate) return;  // Our own correction — skip
            final boolean want = Boolean.TRUE.equals(val);
            userEditable = want;       // Remember design-time intent
            final boolean effective = want && WritablePVWidget.isGlobalWriteEnabled();
            if (effective != want)
            {
                applyingGate = true;
                try { editable.setValue(effective); }
                finally { applyingGate = false; }
            }
        });
        // Register for future enable/disable notifications
        WritablePVWidget.addGateListener(gateListener);
    }
